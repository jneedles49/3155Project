...
def get_project(project_id):
    ...
    if session.get('user'):
        ...
        # create a comment form object
        form = CommentForm()
        return render_template('projects.html', project=my_project, user=session['user'], form=form)
    else:
        return redirect(url_for('login'))
...
def new_note():
    ...
    if session.get('user'):
        ...
        # user is not in session redirect to log in
        return redirect(url_for('login'))
...
def new_comment(note_id, project_id):
    if session.get('user'):
        ...
        if comment_form.validate_on_submit():
            ...
            new_record = Comment(comment_text, int(note_id), int(project_id), session['user_id', 'project_id'])
            db.session.add(new_record)
            db.session.commit()

        return redirect(url_for('get_note', note_id=note_id, project_id=project_id))
